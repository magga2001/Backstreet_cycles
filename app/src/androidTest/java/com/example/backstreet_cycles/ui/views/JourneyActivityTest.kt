...
class JourneyActivityTest{
    ...
    @get:Rule
    var activityRule: ActivityScenarioRule<JourneyActivity> =
        ActivityScenarioRule(JourneyActivity::class.java)
...
//        onView(withId(R.id.prices)).check(matches(isDisplayed()))
    ...
    fun test_distance_image_field_displayed(){
        onView(withId(R.id.DistanceImage)).check(matches(isDisplayed()))
    }
    ...
    fun test_finish_not_clickable(){
        onView(withId(R.id.bottom_sheet_journey)).perform(scrollTo(), click())
